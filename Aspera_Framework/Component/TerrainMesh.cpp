...
TerrainMesh::TerrainMesh() : Mesh(98, "TERRAIN_MESH")
{
	m_terrainFilename = 0;
	m_colorMapFilename = 0;
	m_heightMap = 0;
	m_terrainModel = 0;
	m_meshType = MeshType::TERRAIN_MESH;
}
...
bool TerrainMesh::Initialize(char* setupFilename)
{
	bool result;
...
bool TerrainMesh::LoadSetupFile(char* filename)
{
	int stringLength;
	ifstream fin;
	char input;
...
bool TerrainMesh::LoadBitmapHeightMap()
{
	int error, imageSize, i, j, k, index;
	FILE* filePtr;
	unsigned long long count;
...
bool TerrainMesh::LoadRawHeightMap()
{
	int error, i, j, index;
	FILE* filePtr;
...
bool TerrainMesh::BuildTerrainModel()
{
...
void TerrainMesh::ShutdownTerrainModel()
{
...
void TerrainMesh::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	unsigned int stride;
	unsigned int offset;
