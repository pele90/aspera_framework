...
bool Terrain::Initialize(char* setupFilename)
{
	bool result;
...
bool Terrain::InitializeTerrainCells()
{
	int cellRowCount;

	// Calculate the number of cells needed to store the terrain data.
	cellRowCount = (m_terrainWidth - 1) / (CELL_WIDTH - 1);
...
{
	int index;
	int cellRowCount = (m_terrainWidth - 1) / (CELL_WIDTH - 1);
	bool result;
	...
	for (int j = 0; j < cellRowCount; j++)
	{
		for (int i = 0; i < cellRowCount; i++)
		{
			...
			result = m_terrainCells[index].Initialize(device, m_terrainModel, i, j, CELL_HEIGHT, CELL_WIDTH, m_terrainWidth);
			if (!result)
			{
				return false;
			}
...
bool Terrain::LoadTerrainCellsEx(ID3D11Device* device)
{
